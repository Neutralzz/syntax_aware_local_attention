...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, encoder_hidden_states=None, encoder_attention_mask=None,
                     scaled_attention_mask=None, selector_outputs=None):
...
class BertAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, encoder_hidden_states=None, encoder_attention_mask=None, scaled_attention_mask=None, selector_outputs=None):
        self_outputs = self.self(hidden_states, attention_mask, head_mask, encoder_hidden_states, encoder_attention_mask, scaled_attention_mask, selector_outputs)
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, encoder_hidden_states=None, encoder_attention_mask=None,
        ...
        self_attention_outputs = self.attention(hidden_states, attention_mask, head_mask, 
                                    scaled_attention_mask=scaled_attention_mask, selector_outputs=selector_outputs)
...
class DepAttentionScale(nn.Module):
    def __init__(self, config):
        super(DepAttentionScale, self).__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        self.activation = nn.Tanh()
        self.reg = nn.Linear(config.hidden_size, config.num_attention_heads)
        self.scale_func = config.scale_func

    def forward(self, hidden_states, dep_dist_matrix):
        # dep_dist_matrix (batch_size, num_heads, seq_length, seq_length)
        pooled_output = self.dense(hidden_states) # (batch_size, seq_length, hidden_size)
        pooled_output = self.activation(pooled_output) 
        reg_output = self.reg(pooled_output) # (bs, seq, heads)
        reg_output = reg_output.transpose(-1, -2) # (bs, heads, seq)
        reg_output = reg_output[:,:,:,None]

        if self.scale_func == 'tanh':
            scale_result = 0.5 + 0.5* torch.tanh(reg_output - dep_dist_matrix)  # (batch_size, seq_length, seq_length)
        elif self.scale_func == 'sigmoid':
            scale_result = torch.sigmoid(reg_output - dep_dist_matrix)  # (batch_size, seq_length, seq_length)
        else:
            raise NotImplementedError('Scale Function (%s) not found.'%self.scale_func)

        return scale_result # (bs, heads, seq, heads)
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.num_attention_heads = config.num_attention_heads
        self.scale_layer = nn.ModuleList([DepAttentionScale(config) for _ in range(config.num_hidden_layers)])
        self.selector = nn.ModuleList([AttentionSelector(config) for _ in range(config.num_hidden_layers)])

    def forward(self, hidden_states, attention_mask=None, head_mask=None, encoder_hidden_states=None, encoder_attention_mask=None, dep_dist_matrix=None):
        dep_dist_matrix = dep_dist_matrix[:,None,:,:].expand(-1, self.num_attention_heads, -1, -1)
        all_hidden_states = ()
        all_attentions = ()
        for i, (layer_module, scale_module, selector_module) in enumerate(zip(self.layer, self.scale_layer, self.selector)):
            ...
            scaled_attention_mask = scale_module(hidden_states, dep_dist_matrix)
            selector_outputs = selector_module(hidden_states)
            extended_selector_outputs = selector_outputs[:, :, :, None]
            layer_outputs = layer_module(hidden_states, attention_mask, head_mask[i],
                                    encoder_hidden_states, encoder_attention_mask, scaled_attention_mask, extended_selector_outputs)
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None,
                head_mask=None, inputs_embeds=None, encoder_hidden_states=None, encoder_attention_mask=None, dep_dist_matrix=None):
...
class BertForTokenClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None,
                position_ids=None, head_mask=None, inputs_embeds=None, labels=None, dep_dist_matrix=None):
